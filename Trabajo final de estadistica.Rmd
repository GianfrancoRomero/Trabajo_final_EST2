...
La desigualdad es un problema que cada vez cobra más relevancia en la discusión académica. Juzgar el desarrollo de un país solamente por su PBI ya no es suficiente para considerar que tal país está "bien" o "mal". El uso de otras variables, como la desigualdad, son cada vez más consideradas para entender la realidad socio-económica de un país. Por ello, se vuelve necesario conocer las causas de la desigualdad.

Este trabajo busca, en un primer lugar, entender las causas de la desigualdad. Se analizará primero la riqueza promedio, medida a través del PIB per cápita, y su relación con la desigualdad. Luego, se analizará el ingreso fiscal, medido como un porcentaje del PBI, y su relación con la desigualdad.
...
Para poder medir la riqueza de un país, se utilizará el PIB per cápita para medir la mayor o menor riqueza entre países.
...
```{r}
cor(full_data_stan[,-1])
```

Debido a que la variable del índice GINI es negativa con respecto al resto de variables, invertiremos la variable.
...
Se observa que el método de Diana es el que tiene un silhouette width mayor que los otros. Por ello, optamos por utilizar Diana para hacer los clusters
...
Vemos que Diana es el único método sin datos mal clusterizados. Esto refureza nuestra decisión de utilizar este método.
...
Antes se proponían 3 factores para las variables disponibles. Sin embargo, ahora podemos ver que las variables del índice de GINI y el PBI de cada país son tan diferentes de las otras variables que no podrían formar un índice con las demás.
